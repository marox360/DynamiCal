...
namespace DynamiCal.DataGridView
{
    public class DataGridViewDayCell : DataGridViewTextBoxCell
    {
        ...
        protected override void Paint(System.Drawing.Graphics graphics, System.Drawing.Rectangle clipBounds, System.Drawing.Rectangle cellBounds, int rowIndex, DataGridViewElementStates cellState, object value, object formattedValue, string errorText, DataGridViewCellStyle cellStyle, DataGridViewAdvancedBorderStyle advancedBorderStyle, DataGridViewPaintParts paintParts)
        {
            ...
            else if (dayValue.IsSameDayOf(DateTime.Today))
            {
            ...
            base.Paint(graphics, clipBounds, dayValue.IsWeekendDay() ? cellBounds : borderCellBounds, rowIndex, cellState, value, dayValue.Description, errorText, cellStyle, advancedBorderStyle, paintParts);

            if (dayValue.IsTodayWeek())
            {
                using (Pen pen = new Pen(dayValue.IsSameDayOf(DateTime.Today) ? Color.FromArgb(192, 85, 87) : Color.FromArgb(244, 203, 204), 2))
                {
                    Rectangle bounds = dayValue.Date.DayOfWeek == DayOfWeek.Monday || dayValue.IsSameDayOf(DateTime.Today) ? borderCellBounds : cellBounds;

                    int y = bounds.Top + (int)pen.Width/2;

                    if (dayValue.Date.DayOfWeek != DayOfWeek.Monday && dayValue.IsSameDayOf(DateTime.Today))
                    {
